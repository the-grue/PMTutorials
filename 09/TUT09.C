...
word total_tasks=1;
word cur_task=0,        /* current task */
     next_task;         /* task to switch to (see isr_30_wrapper) */

byte task_stacks[max_tasks-1][stack_size];      /* task stacks */
FARPOINTER stack_pointers[max_tasks];           /* SS:SP for each task */
...
void setup_IDT() {
  ...
  setup_IDT_entry (&idt[0x30], 0x08, (word)&isr_30_wrapper, ACS_INT, 0);
...
void scheduler() {
  ...
  if ((next_task=cur_task+1) >= total_tasks) next_task = 0;

  /* isr_30_wrapper will push all registers to the current stack and
     save SS:ESP pair to stack_pointers[cur_task]. Then it will
     load SS:ESP pair from stack_pointers[next_task], pop all registers
     and issue IRETD. This will finally switch tasks. */

  __emit__ (0xCD, 0x30);                /* int 30h */
}
...
void task2() {
  char far *scr;
  scr = MK_FP (0x20, 80*12+10*2);
  for(;;)
    scr[0]++;
}

int add_task (word CS, word IP, word DS, word SS, word SP) {
  dword far *p;
  ...
  /* let's push all registers to the new task stack */

  p--;
  *p-- = 0x202L;                        /* EFLAGS (ints enabled) */
  *p-- = CS;                            /* CS */
  *p-- = IP;                            /* EIP */
  *p-- = DS;                            /* DS */
  *p-- = DS;                            /* ES */
  *p-- = 0;                             /* FS */
  *p-- = 0;                             /* GS */
  p -= 6;                               /* EAX, EBX, ECX, EDX, ESI, EDI, EBP */

  /* let's save adjusted SS:SP to stack_pointers[] */

  stack_pointers[total_tasks].offset = FP_OFF(p);
  stack_pointers[total_tasks].selector = SS;

  /* it's now possible to switch to this task popping registers back and
     issuing an IRETD. For more info see isr_30_wrapper. */
...
int main() {
  ...
  clrscr();
  printf ("Welcome to the 9th PMode tutorial!\n\n");
  atexit (my_exit);
  ...
  setup_PMode();
  add_task (0x08, (word)&task1, 0x10, 0x10, (word)&task_stacks[1]);
  add_task (0x08, (word)&task2, 0x10, 0x10, (word)&task_stacks[2]);
