...
void setup_GDT() {
  ...
  tmp = ((dword)_DS)<<4;
  tmp += (word)&ldt[0];
  setup_GDT_entry (&gdt[5], tmp, sizeof(ldt)/2-1, ACS_LDT | ACS_PRESENT, 0);

  /* 0x30 -- LDT for task2() */
  tmp = ((dword)_DS)<<4;
  tmp += (word)&ldt[1];
  setup_GDT_entry (&gdt[6], tmp, sizeof(ldt)/2-1, ACS_LDT | ACS_PRESENT, 0);
...
void setup_LDT() {
  /* 0x04 -- alias 32-bit code segment descriptor for task1() */
  setup_GDT_entry (&ldt[0][0], ((dword)_CS)<<4, 0xFFFF, 
                   ACS_CODE | ACS_DPL_3, ATTR_DEFAULT);

  /* 0x0C -- alias data segment descriptor for task1() */
  setup_GDT_entry (&ldt[0][1], ((dword)_DS)<<4, 0xFFFF, 
                   ACS_DATA | ACS_DPL_3, 0);

  /* 0x04 -- alias 32-bit code segment descriptor for task2() */
  setup_GDT_entry (&ldt[1][0], ((dword)_CS)<<4, 0xFFFF, 
                   ACS_CODE | ACS_DPL_3, ATTR_DEFAULT);

  /* 0x0C -- alias data segment descriptor for task2() */
  setup_GDT_entry (&ldt[1][1], ((dword)_DS)<<4, 0xFFFF, 
                   ACS_DATA | ACS_DPL_3, 0);
}
...
void setup_PMode() {
  ...
  for (i=0;i<total_tasks;i++) {
    ...
    if (i) {
      ...
      tss[i].ds = tss[i].es = tss[i].ss = 0x0C | 3;/* ds=es=ss = data segment */
  ...
  tss[0].ldtr = 0;                              /* ldtr=0 */
  tss[1].ldtr = 0x28;                           /* ldtr=0x28 */
  tss[2].ldtr = 0x30;                           /* ldtr=0x30 */
...
int main() {
  ...
  clrscr();
  printf ("Welcome to the 12th PMode tutorial!\n\n");
  atexit (my_exit);
