...
char msg[]="H e l l o   f r o m   t a s k ( ) !   E S C - r e t u r n   t o   m a i n ( ) . ";
...
void task() {
  ...
  /* far jump to main() -- selector is 0x28 for main() */
  __emit__ (0xEA, 0, 0, 0x28, 0);       /* JMP FAR 0x28:0 */
}
...
int main() {
  clrscr();
  printf ("Welcome to the 6th PMode tutorial!\n\n");
  atexit (my_exit);
  ...
  /* far jump to a task (task()) -- selector is 0x30 for task() */
  __emit__ (0xEA, 0, 0, 0x30, 0);       /* JMP FAR 0x30:0 */
