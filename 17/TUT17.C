...
void setup_GDT() {
  ...
  /* 0x38 -- code segment descriptor for task() */
  setup_GDT_entry (&gdt[7], ((dword)_CS)<<4, 0xFFFF, ACS_CODE | ACS_DPL_3, 0);

  /* 0x40 -- data/stack segment descriptor for task() */
  setup_GDT_entry (&gdt[8], ((dword)_DS)<<4, 0xFFFF, ACS_DATA | ACS_DPL_3, 0);

  /* 0x48 -- call gate */
  setup_IDT_entry ((DESCR_INT *)&gdt[9], 0x08, (word)&call_gate_wrapper, 
                   ACS_CALL16 | ACS_DPL_3, 0);
...
/* but this doesn't :) */
  for(;;)
    __emit__ (0x9A, 0, 0, 0x48, 0);     /* CALL FAR 48h:0 */
}
...
int main() {
  char far *scr = MK_FP (0x20, 80*8+10*2);
  word x1, x2;

  clrscr();
  printf ("Welcome to the 17th PMode tutorial!\n\n");
  atexit (my_exit);
